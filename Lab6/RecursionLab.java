...
*       (1a) sum( 1 ) = 1
*
* and from this definition, we can rewrite this formula in terms of itself, such
that:
*
*       (2) sum( n ) = n + sum( n - 1 )
*
...
public class RecursionLab {
    ...
    public static void main( String args[] ) { //invoke the recursive method here...
        /**
        * switch between the two commented lines below and execute this
        code,
        * observing the output for both the iterative solution and the
        recursive solution.
        * To watch the recursive behaviour in action, set a breakpoint on the
        if statement
    ...
    public static int factorial(int n){
        if (n == 1 || n == 0) {
            ...
            return 1; //for summation, factorials, and exponent this is 1
            
        } else {
            
            return n * factorial(n-1); //typically the recursive call decrements by 1 or n/2
            
        }
    }

    public static int powerIncremental(int n, int x){
        if (n == 0) { 
            return 1; // Base case 
        } else {   
            return x * powerIncremental(x, n - 1);
        }
    }

    public static int powerDivide(int n, int x){
        if (n==0){
            return 1;
        }
        if (n%2 == 0){
            int halfPower = powerDivide(x, n / 2); 
            return halfPower * halfPower;
        } else {
            int halfPower = powerDivide(x, (n-1) / 2); 
            return halfPower * halfPower;
